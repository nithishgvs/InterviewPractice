...
public class ConcurrentLRUCacheConfluent<K, V> {
  ...
  public ConcurrentLRUCacheConfluent(int capacity) {
    ...
    try {
      if (cache.containsKey(key)) {
        ...
        if (cache.size() >= capacity) {
          Node<K, V> removedNode = list.removeFromEnd();
          if (removedNode != null) {
  ...
  public static void main(String[] args) {
    ConcurrentLRUCacheConfluent<Integer, String> cache = new ConcurrentLRUCacheConfluent<>(3);
    cache.put(1, "one");
    cache.put(2, "two");
    cache.put(3, "three");
